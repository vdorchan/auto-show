(function () {
    ...
    class Controller extends Laya.Script {
        ...
        onAwake() {
            ...
            this.controller = this.owner.getChildByName('controller');
            this.controller.pos(20, Laya.stage.height - this.controller.height - 20);
            this.controllerDrag = this.controller.getChildByName('controller_drag');
        ...
        onRockerMouseMove(e) {
            if (!this.isDown || e.touchId != this.curTouchId)
                return;
            var locationPos = this.controllerDrag.globalToLocal(new Laya.Point(Laya.stage.mouseX, Laya.stage.mouseY), false);
    ...
    var panCarConfig = panoCars.map(config => ({ color: config.ColorValue, list: config.Hori.Normal.map(n => `res/pano-cars/${config.ColorValue}/${n.Url.match(/[^\/]+\.png$/g)[0]}`) }));
    ...
    class GameUI extends ui.Hall {
        ...
        moveCharacter() {
            if (this.controller && this.controller.angle !== -1) {
                ...
                Laya.KeyBoardManager.hasKeyDown(87) && this.camera.transform.translate(this.translateW);
                Laya.KeyBoardManager.hasKeyDown(83) && this.camera.transform.translate(this.translateS);
                Laya.KeyBoardManager.hasKeyDown(65) && this.camera.transform.translate(this.translateA);
                Laya.KeyBoardManager.hasKeyDown(68) && this.camera.transform.translate(this.translateD);
            }
        }
        createLight() {
    ...
    class GameConfig {
        ...
        static init() {
            ...
            reg("script/Controller.ts", Controller);
        }
    }
